...
def display_status(epd, status, onion_address, name, email, key_id, expires):
    ...
    # Add the new text
    font_instruction = ImageFont.truetype(
    ...
    for line in wrapped_instruction:
        draw.text(
            (x_pos_status, y_pos_instruction), line, font=font_instruction, fill=0
        )
        y_pos_instruction += (
            font_instruction.getbbox(wrapped_instruction[-1])[3]
            - font_instruction.getbbox(wrapped_instruction[-1])[1]
            + 5
        )
    ...
    new_size = (
        int(qr_img.width * width_scale_factor),
        int(qr_img.height * height_scale_factor),
    )
    ...
    chars_per_line = max_width // (
        font_info.getbbox("A")[2] - font_info.getbbox("A")[0]
    )
